...
Now that you have published the SalesOrder API to Mashery, you will adjust the configuration of the API definition and the API package of
...
4. Navigate to the **Developer Portal**, and click on **API Console**. In the new screen, select **SalesOrderAPI**, and click on the **Try it out** button. Fill out the values for the **orderId** and the **api_key**, and click the **Execute** button.
    ...
    If everything goes well, you'll see same response as in the previous test with Postman.
